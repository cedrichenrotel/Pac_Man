...
class Draw:
    ...
    def _paste_wall_segment(
        self, x: float, y: float, dx: int, dy: int
    ) -> None:
        ...
        for i in range(0, self.cell_size, wall_width):
            if dx == 0:
                ...
                self.canvas.paste(
                    self.wall_sprite, (px + i, py_pos), self.wall_sprite
                )
            else:
                if dx == -1:
                ...
                self.canvas.paste(
                    self.wall_sprite, (px_pos, py + i), self.wall_sprite
                )
    ...
    def draw_maze(self) -> bool:
        ...
        self.maze_img_ptr: int = self._pil_to_mlx_image(
            self.canvas, "maze_cache.png"
        )
        return True
    ...
    def draw_pacman(self) -> None:
        ...
        sprite_name: str = direction_sprites.get(
            self.pacman.key_direction or "E", "pacman_chomp"
        )
    ...
    def draw_ghost(self) -> None:
        ...
        color_ghost: dict[str, str] = {"R": "ghost_red", "B": "ghost_blue"}
